...
# Creates the Blog example domain and the email_templates table.
# This reproduces db/schema.rb; the canonical setup path is `bin/rails db:schema:load`.
class CreateBlogAndEmailTables < ActiveRecord::Migration[8.1]
  def change
    create_table :blog_category, comment: "블로그 카테고리 테이블 (2단계 계층)" do |t|
    ...
    create_table :blog_post, comment: "블로그 게시글 테이블" do |t|
    ...
    create_table :blog_post_category, primary_key: [:blog_post_id, :category_id], comment: "블로그 게시글-카테고리 매핑 테이블" do |t|
      t.bigint :blog_post_id, null: false
      t.bigint :category_id, null: false
    end

    create_table :blog_view, comment: "블로그 조회 기록 테이블" do |t|
      t.bigint :blog_post_id, null: false
    ...
    create_table :blog_author_permission, comment: "블로그 작성 권한 관리 테이블" do |t|
      t.uuid :author_id, null: false
      t.string :author_type, limit: 20, null: false
      t.datetime :created_at, precision: nil, default: -> { "CURRENT_TIMESTAMP" }, null: false
    ...
    create_table :email_templates, primary_key: "key", id: { type: :string, limit: 50 } do |t|
